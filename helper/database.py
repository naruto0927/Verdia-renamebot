...
class Database:
    ...
    async def set_premium(self, user_id: int, value: bool, expiry=None):
        """Grant (value=True) or revoke (value=False) premium.
        expiry is a Unix timestamp (float) or None for lifetime."""
        await self.col.update_one(
            {'_id': int(user_id)},
            {'$set': {'premium': value, 'premium_expiry': expiry}},
            upsert=True,
        )

    async def is_premium(self, user_id: int) -> bool:
        """Return True if user has active non-expired premium, or is an admin."""
        import time
        from config import Config
    ...
    async def get_premium_info(self, user_id: int):
        """Return the raw user document (for /premium status display)."""
        return await self.col.find_one({'_id': int(user_id)})

    async def get_all_premium_users(self):
        """Cursor of all users with premium=True."""
